namespace EsrNhsHubFileTransferService.Code.Database
{
    ...
    {
        private static readonly string EntityConnectionString = string.Empty;
        ...
        static EsrNhsHubDatabase()
        {
            ...
            {
                EventLog.WriteEntry("_File Transfer Service - ESR NHS Hub", string.Format("Exception whilst decrypting and creating a new entity connection string, check the esrNhsHubEntities connection string in the config has the correctly encrypted password.\n\nThe service cannot process files until this is corrected and the service is restarted.\n\n{0}", exception.Message), EventLogEntryType.Error);
                throw;
            }
        }
    }
