namespace EsrGo2FromNhsWcfLibrary.Crud
{
    ...
    public class LogDbConnection : IApiDbConnection
    {
        ...
        /// </summary>
        public ApiLog.MessageLevel DebugLevel { get; set; }
        ...
        public int ExecuteProc(string sql)
        {
            ...
            {
                string sqlString = sql.Length > 4000 ? sql.Substring(0, 4000) : sql;
                EventLog.WriteEntry(
                    "Application",
                    string.Format("Execute Proc error \n {0} \n {1} \n SQL = {2}", ex.Message, ex.StackTrace, sqlString));
                Debug.WriteLine(ex);
        ...
        {
            var decryptor = new System.Security.Cryptography.RijndaelManaged();
            var stream = new System.IO.MemoryStream();
            byte[] inputByteArray = Convert.FromBase64String(data);
            var cs = new System.Security.Cryptography.CryptoStream(stream, decryptor.CreateDecryptor(Key, Iv), System.Security.Cryptography.CryptoStreamMode.Write);
        ...
        /// </returns>
        private ApiLog.MessageLevel GetEsrDiagnostics()
        {
            ...
            using (IDataReader reader = this.GetReader(this.ConnectionString, Strsql))
            {
                ...
                while (reader.Read())
                {
                    ...
                    this.Cache.Add("messageLevel", stringValue, new CacheItemPolicy { SlidingExpiration = TimeSpan.FromMinutes(15) });
                }
            }

            return (ApiLog.MessageLevel)stringValue;
        }
        ...
        Log.MessageLevel IApiDbConnection.DebugLevel
        {
            get
            {
                throw new NotImplementedException();
