namespace EsrGo2FromNhsWcfLibrary.ESR
{
    ...
    public class EsrHeaderRecord
    {
        ...
        /// </summary>
        private readonly string vpdNumber;
        ...
        public static EsrHeaderRecord ParseEsrHeaderRecord(string recordLine)
        {
            ...
            if (EsrDateHelpers.TryParseExact(rec[(int)HeaderRecordColRef.CreationDate], "yyyyMMdd HHmmss", out tmpDate))
            {
                creationDate = tmpDate;
            }
            string trustvpd = rec[(int)HeaderRecordColRef.TrustIdentifier];
            DateTime prevrundate = DateTime.MinValue;
            if (EsrDateHelpers.TryParseExact(rec[(int)HeaderRecordColRef.PreviousRunDate], "yyyyMMdd HHmmss", out tmpDate))
            {
                prevrundate = tmpDate;
            }

            DateTime rundate = DateTime.MinValue;
            if (EsrDateHelpers.TryParseExact(rec[(int)HeaderRecordColRef.RunDate], "yyyyMMdd HHmmss", out tmpDate))
            {
                rundate = tmpDate;
            }
