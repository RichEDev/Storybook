        ...
        public static EsrAddressRecord ParseEsrAddressRecord(string recordLine)
        {
            ...
            switch (rec[(int)AddressRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrAddressUpdateRecordType:
                    if (rec.Length != ((int)AddressRecordColumnsRef.LastUpdateDate + 1)) // insufficient columns in row
                    {
                        return null;
                    }
                    ...
                    DateTime tmpDate;
                    if(EsrDateHelpers.TryParseExact(rec[(int)AddressRecordColumnsRef.EffectiveStartDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveStartDate = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)AddressRecordColumnsRef.EffectiveEndDate], "yyyyMMdd",  out tmpDate))
                    {
                        effectiveEndDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)AddressRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", out tmpDate))
                    {
