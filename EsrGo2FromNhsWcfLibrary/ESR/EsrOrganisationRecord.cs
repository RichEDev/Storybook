        ...
        public static EsrOrganisationRecord ParseEsrOrganisationRecord(string recordLine)
        {
            ...
            switch (rec[(int)OrganisationRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrOrganisationUpdateRecordType:
                    ...
                    DateTime tmpDate;
                    if (EsrDateHelpers.TryParseExact(rec[(int)OrganisationRecordColumnsRef.EffectiveFromDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveFromDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)OrganisationRecordColumnsRef.EffectiveToDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveToDate = tmpDate;
                    }
                    long.TryParse(rec[(int)OrganisationRecordColumnsRef.HierarchyVersionId], out hierarchyVersionId);
                    if (EsrDateHelpers.TryParseExact(rec[(int)OrganisationRecordColumnsRef.HierarchyVersionDateFrom], "yyyyMMdd", out tmpDate))
                    {
                        hierarchyVersionDateFrom = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)OrganisationRecordColumnsRef.HierarchyVersionDateTo], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)OrganisationRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", out tmpDate))
                    {
