        ...
        public EsrAssignment PrimaryAssignment
        {
            get
            {
                ...
                switch (list.Count)
                {
                    ...
                    default:
                    {
                        var latestEffectiveStartDate = this.EsrAssignments.Max(assignment => assignment.EffectiveStartDate);
        ...
        public static EsrPersonRecord ParseEsrPersonRecord(string recordLine)
        {
            ...
            switch (rec[(int)PersonRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrPersonUpdateRecordType:
                    ...
                    recordAction = Base.Action.Update;
                    long.TryParse(rec[(int)PersonRecordColumnsRef.PersonId], out esrPersonId);
                    DateTime tmpDate;
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.EffectiveStartDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveStartDate = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.EffectiveEndDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.DateOfBirth], "yyyyMMdd", out tmpDate))
                    {
                        dateOfBirth = tmpDate;
                    }
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.HireDate], "yyyyMMdd", out tmpDate))
                    {
                        hireDate = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.ActualTerminationDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.WtrOptOutDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.Csd3Months], "yyyyMMdd", out tmpDate))
                    {
                        csd3Months = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.Csd12Months], "yyyyMMdd", out tmpDate))
                    {
                        csd12Months = tmpDate;
                    }
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.NhsStartDate], "yyyyMMdd", out tmpDate))
                    {
                        nhsStartDate = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)PersonRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", out tmpDate))
                    {
