namespace EsrGo2FromNhsWcfLibrary.ESR
{
    ...
    public class EsrPhoneRecord : EsrPhone
    {
        ...
        public static EsrPhoneRecord ParseEsrPhoneRecord(string recordLine)
        {
            ...
            var esrLastUpdateDate = new DateTime(1900, 1, 1);
            ...
            switch (rec[(int)PhoneRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrPhoneUpdateRecordType:
                    ...
                    DateTime tmpDate;
                    if (EsrDateHelpers.TryParseExact(rec[(int)PhoneRecordColumnsRef.EffectiveStartDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveStartDate = tmpDate;
                    }

                    if (EsrDateHelpers.TryParseExact(rec[(int)PhoneRecordColumnsRef.EffectiveEndDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveEndDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)PhoneRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", out tmpDate))
                    {
