namespace EsrGo2FromNhsWcfLibrary.ESR
{
    ...
    public class EsrAssignmentRecord : EsrAssignment
    {
        ...
        /// </param>
        /// <param name="departmentManagerAssignmentId">
        ///     The department manager assignment id.
        /// </param>
        ...
        public EsrAssignmentRecord(int sysInternalEsrAssignId, long? esrPersonId, long assignmentId, DateTime effectiveStartDate, DateTime? effectiveEndDate, DateTime earliestStartDate, string assignmentType, string assignmentNumber, string systemAssignmentStatus, string assignmentStatus, string employeeStatusFlag, string payrollName, string payrollPeriodType, long? esrLocationId, string supervisorFlag, long? supervisorPersonId, long? supervisorAssignmentId, string supervisorAssignmentNumber, long? departmentManagerAssignmentId, string employeeCategory, string assignmentCategory, bool primaryAssignment, string primaryAssignmentString, decimal normalHours, string normalHoursFrequency, decimal gradeContractHours, decimal fte, string flexibleWorkingPattern, long? esrOrganisationId, long? esrPositionId, string positionName, string grade, string gradeStep, DateTime? startDateInGrade, decimal annualSalaryValue, string jobName, string peopleGroup, string tAndAFlag, string assignmentNightWorkerAttribute, DateTime? projectedHireDate, int? vacancyId, DateTime? contractEndDate, DateTime? incrementDate, string maximumPartTimeFlag, string afcFlag, DateTime? esrLastUpdate, DateTime? lastWorkingDay, string eKsfFSpinalPoint, string managerFlag, DateTime? assignmentEndDate)
        {
            ...
            this.DepartmentManagerAssignmentId = departmentManagerAssignmentId;
        ...
        public static EsrAssignmentRecord ParseEsrAssignmentRecord(string recordLine)
        {
            ...
            const int SysInternalEsrAssignId = 0;
            long esrPersonId = 0;
            long assignmentId;
            var effectiveStartDate = new DateTime(1900, 1, 1);
            DateTime? effectiveEndDate = null;
            var earliestStartDate = new DateTime(1900, 1, 1);
            ...
            long? departmentManagerAssignmentId = null;
            ...
            switch (rec[(int)AssignmentRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrAssignmentUpdateRecordType:
                    ...
                    DateTime tmpDate;
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.EffectiveStartDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveStartDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.EffectiveEndDate], "yyyyMMdd", out tmpDate))
                    {
                        effectiveEndDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.EarliestAssignmentDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (long.TryParse(rec[(int)AssignmentRecordColumnsRef.DepartmentManagerAssignmentId], out tmpId))
                    {
                        departmentManagerAssignmentId = tmpId;
                    }
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.StartDateInGrade], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.ProjectedHireDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.ContractEndDate], "yyyyMMdd", out tmpDate))
                    {
                        contractEndDate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.IncrementDate], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", out tmpDate))
                    {
                        esrLastUpdate = tmpDate;
                    }
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.LastWorkingDay], "yyyyMMdd", out tmpDate))
                    {
                    ...
                    if (EsrDateHelpers.TryParseExact(rec[(int)AssignmentRecordColumnsRef.AssignmentEndDate], "yyyyMMdd", out tmpDate))
                    {
            ...
            return new EsrAssignmentRecord(
                ...
                {
                    Action = recordAction,
                    SafeDepartmentManagerAssignmentId = departmentManagerAssignmentId,
        ...
        {
            if (record.DepartmentManagerAssignmentId == null)
            {
                record.DepartmentManagerAssignmentId = record.SafeDepartmentManagerAssignmentId;
            }
