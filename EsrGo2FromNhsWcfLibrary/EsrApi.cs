namespace EsrGo2FromNhsWcfLibrary
{
    ...
    public class EsrApi : IEsrApi
    {
        ...
        public List<T> Execute<T>(DataAccessMethod accessMethod, string identifier = "", List<T> entities = null, DataAccessMethodReturnDefault returnDefault = DataAccessMethodReturnDefault.NewObject) where T : DataClassBase, new()
        {
            ...
            switch (accessMethod)
            {
                case DataAccessMethod.Create:
                    ...
                    if (type == typeof(Employee) || type == typeof(EsrAssignment) || type == typeof(EsrAssignmentCostings))
                    {
                        clearingCache = true;
                    }
                    break;
                case DataAccessMethod.Update:
                    ...
                    if (type == typeof(Employee) || type == typeof(EsrAssignment))
                    {
                        clearingCache = true;
                    }
                ...
                case DataAccessMethod.ReadByEsrId:
                    ...
                    break;
                case DataAccessMethod.ReadByAssignmentId:
                    result = dataAccess.ReadByAssignmentId(this._entityId);
                    break;
            ...
            if (clearingCache)
            {
                this.ClearCache(result.ToArray());
            }
        ...
        {
            this.CheckMetaBase(this._metabase);
            this._logger.WriteDebug(this._metabase, esrTrust, this._accountId, LogRecord.LogItemTypes.None, LogRecord.TransferTypes.EsrOutbound, 0, string.Empty, LogRecord.LogReasonType.None, string.Format("{0} - {1}", "FindEsrTrust", DateTime.Now), "ApiCrud");
        ...
        /// <summary>
        /// Remove the specified entities from the cache.
        /// </summary>
        /// <param name="entities">
        /// The collection of items to be removed.
        /// </param>
        public void ClearCache<T>(params T[] entities)
        {
            const string employeeKey = "employee";
            const string assignmentKey = "esrAssignments";
            const string costcodeBreakdownKey = "employeeCostBreakdown";

            var employees = entities as Employee[];

            if (employees != null)
            {
                foreach (Employee dataClassBase in employees)
                {
                    DeleteCacheKey(employeeKey, dataClassBase.employeeid);
                }
            }

            var assignments = entities as EsrAssignment[];
         
            if (assignments != null)
            {
                foreach (EsrAssignment ass in assignments)
                {
                    DeleteCacheKey(assignmentKey, ass.employeeid);
                }
            }

            var assignmentCostings = entities as EsrAssignmentCostings[];

            if (assignmentCostings != null)
            {
                foreach (EsrAssignmentCostings costing in assignmentCostings)
                {
                    //Get EmployeeId from ESRPersonId
                    var employee = this.Execute<Employee>(DataAccessMethod.ReadByEsrId, costing.ESRPersonId.ToString(CultureInfo.InvariantCulture));

                    if (employee.Count == 1)
                    {
                        DeleteCacheKey(employeeKey, employee[0].employeeid);
                        DeleteCacheKey(costcodeBreakdownKey, employee[0].employeeid);     
                    }
        ...
        /// <summary>
        /// Deletes a cache entry based on the account, key and employeeId
        /// </summary>
        /// <param name="keyName"></param>
        /// <param name="employeeId"></param>
        private void DeleteCacheKey(string keyName, int employeeId)
        {
            _cache.Delete(this._accountId, keyName, employeeId.ToString(CultureInfo.InvariantCulture));
        }
