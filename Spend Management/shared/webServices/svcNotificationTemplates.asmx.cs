...
namespace Spend_Management
{
    ...
    /// <summary>
    /// Summary description for svcNotificationTemplates
    /// </summary>
    ...
    public class svcNotificationTemplates : WebService
    {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod]
        public int SaveNotificationTemplate(int emailtemplateid, string templatename, string toVal, string ccVal, string bccVal, string subject, byte priority, string body, Guid basetableid, bool? sysTemp, bool issendnote, string notes, bool update, bool isSendEmail, bool canSendMobileNotification, string mobileNotificationMessage)
        {
            int id = 0;
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (ccVal != "")
                {
                    notifications.getRecipientEmailFields(ref lstRecipientTypes, ccVal, recipientType.cc, basetableid);
                }

                if (bccVal != "")
                {
                    notifications.getRecipientEmailFields(ref lstRecipientTypes, bccVal, recipientType.bcc, basetableid);
                }

                buildSubject.details = WebUtility.HtmlDecode(subject);
                buildSubject.fieldDetails = notifications.getFieldsFromText(subject, basetableid, user);
                buildBody.details = WebUtility.HtmlDecode(body);
                buildBody.fieldDetails = notifications.getFieldsFromText(body, basetableid, user);
                buildNote.details = WebUtility.HtmlDecode(notes);
                buildNote.fieldDetails = notifications.getFieldsFromText(notes, basetableid, user);

                if (sysTemp == null)
                {
                    var emailTemplate = notifications.GetNotificationTemplateById(emailtemplateid);
                ...
                {
                    var oldTemplate = notifications.GetNotificationTemplateById(emailtemplateid);
                ...
                {
                    notifications.getRecipientEmailFields(ref lstRecipientTypes, toVal, recipientType.to, basetableid);
                }
                if (emailtemplateid == 0)
                {
                    notificationTemplate = new NotificationTemplate(emailtemplateid, templatename, lstRecipientTypes, buildSubject, buildBody, isSystemTemplate, emailPriority, basetableid, DateTime.UtcNow, user.EmployeeID, null, null, isSendEmail, true, null, issendnote, buildNote, null, canSendMobileNotification, mobileNotificationMessage);
                }
                else
                {
                    notificationTemplate = new NotificationTemplate(emailtemplateid, templatename, lstRecipientTypes, buildSubject, buildBody, isSystemTemplate, emailPriority, basetableid, new DateTime(1900, 01, 01), 0, DateTime.UtcNow, user.EmployeeID, isSendEmail, true, null, issendnote, buildNote, null, canSendMobileNotification, mobileNotificationMessage);
                }
               
                id = notifications.SaveNotificationTemplate(notificationTemplate, update);
            }
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod]
        public void DeleteNotifcationTemplate(int notificationTemplateId)
        {
            if (User.Identity.IsAuthenticated)
            {
                CurrentUser user = cMisc.GetCurrentUser();
                NotificationTemplates notifications = new NotificationTemplates(user);
                notifications.DeleteNotificationTemplate(notificationTemplateId);
            }
        }
    }
