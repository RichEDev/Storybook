...
namespace Spend_Management
{
    ...
    public class cEmailTemplates : IEmailTemplates
    {
        ...
        SortedList<int, cEmailTemplate> list;
        ...
        public cEmailTemplates(ICurrentUser user)
        {
        ...
        public cEmailTemplates(ICurrentUser user, string alternateEmailId)
        {
            this.nAccountid = user.AccountID;
            this.currentUser = user;
            this.joinVias = new JoinVias(this.currentUser);
        ...
        /// <summary>
        /// Create a new cEmailTemplates for Employee locked / password request / Employee activated through ESR where current user cannot be obtained.
        /// </summary>
        /// <param name="accountId">The current account ID</param>
        ...
        public cEmailTemplates(int accountId, int employeeId, string passwordKey, int attempts, Modules currentModule,bool fromEsr = false)
        {
        ...
        private SortedList<int, cEmailTemplate> CacheList()
        {
            var expdata = new DatabaseConnection(cAccounts.getConnectionString(accountid));
            SortedList<int, cEmailTemplate> lstEmailTemplates = new SortedList<int, cEmailTemplate>();
            int emailtemplateid;
            bool systemTemplate;
        ...
        public cEmailTemplate getEmailTemplateById(int id)
        {
            cEmailTemplate temp = null;
        ...
        {
            SortedList<int, cEmailTemplate> sorted = list;
            List<ListItem> items = new List<ListItem>();

            foreach (cEmailTemplate temp in sorted.Values)
            {
        ...
        /// </summary>
        /// <param name="emailTemp">Email template object</param>
        /// <param name="update">boolean that checks if it is an insert or update of the email template to the database</param>
        /// <returns></returns>
        public int saveEmailTemplate(cEmailTemplate emailTemp, bool update)
        {
            ...
            {
                if (!update && emailTemp.EmailTemplateId == 0)
                {
                    if (this.list.Any(e => e.Value.TemplateName.Trim().ToLower() == emailTemp.TemplateName.Trim().ToLower()))
                    {
                ...
                expdata.sqlexecute.Parameters.AddWithValue("@emailtemplateid", emailTemp.EmailTemplateId);
                ...
                if (emailTemp.TemplateName == string.Empty)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@templatename", DBNull.Value);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@templatename", emailTemp.TemplateName);
                }
                if (emailTemp.Subject.details == string.Empty)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@subject", DBNull.Value);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@subject", emailTemp.Subject.details);
                }

                if (emailTemp.Body.details == string.Empty)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@bodyhtml", DBNull.Value);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@bodyhtml", WebUtility.HtmlDecode(emailTemp.Body.details));
                }

                if (emailTemp.Note.details == string.Empty)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@note", DBNull.Value);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@note", WebUtility.HtmlDecode(emailTemp.Note.details));
                }

                expdata.sqlexecute.Parameters.AddWithValue("@priority", emailTemp.Priority);
                expdata.sqlexecute.Parameters.AddWithValue("@basetableid", emailTemp.BaseTableId);
                expdata.sqlexecute.Parameters.AddWithValue("@systemtemplate", emailTemp.SystemTemplate);

                expdata.sqlexecute.Parameters.AddWithValue("@date", DateTime.Now);
                if (emailTemp.ModifiedBy == null)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@userid", emailTemp.CreatedBy);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@userid", emailTemp.ModifiedBy);
                }

                if (emailTemp.SendNote == null)
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@sendnote", DBNull.Value);
                }
                else
                {
                    expdata.sqlexecute.Parameters.AddWithValue("@sendnote", emailTemp.SendNote);
                }

                if (emailTemp.SendEmail == null)
                {
                ...
                int emailtemplateid = (int)expdata.sqlexecute.Parameters["@identity"].Value;
                expdata.sqlexecute.Parameters.Clear();

                this.saveRecipients(emailTemp.RecipientTypes, emailtemplateid);
                this.saveEmailFields(emailTemp.Subject.fieldDetails, emailTemplateSection.subject, emailtemplateid);
                this.saveEmailFields(emailTemp.Body.fieldDetails, emailTemplateSection.body, emailtemplateid);
                this.saveEmailFields(emailTemp.Note.fieldDetails, emailTemplateSection.note, emailtemplateid);
        ...
        {
            cEmailTemplate emailTemp = getEmailTemplateById(emailtemplateid);
        ...
        public void SendMessage(Guid? templateId, int senderid, int[] recipientsId, object filterfieldval = null, bool forceSend = false, string defaultSender = null, Guid? filterFieldGuid = null, DataTable EmployeeDetailsForBody = null )
        {
            ...
            {
                cEmailTemplate reqmsg = this.Get(templateId);
        ...
        public void processEmail(cEmailTemplate emailTemp, List<sSendDetails> lstRecipientTypes, cField entityfield, int filterval, int senderid, string bodyUpdate)
        {
            if (emailTemp != null)
            {
        ...
        /// </param>
        private void ProcessEmailWithToken(cEmailTemplate emailTemplate, cField entityField, object filterValue, int senderId, string bodyUpdate, int recieverId = 0, string defaultSender = null, DataTable employeeDetailsForBody = null)
        {
        ...
        public string ProcessNoteWithToken(cEmailTemplate emailTemplate, cField entityField, object filterValue, int senderId, int receiverId = 0)
        {
        ...
        /// <returns></returns>
        public cEmailTemplate Get(Guid? templateid)
        {
