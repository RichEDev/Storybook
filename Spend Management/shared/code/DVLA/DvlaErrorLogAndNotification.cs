...
namespace Spend_Management.shared.code.DVLA
{
    ...
    public class DvlaServiceEventLogAndEmailNotification
    {
        ...
        /// <param name="emailTemplates">The <see cref="cEmailTemplates"></see></param>
        /// <param name="employee">The <see cref="Employee"></see></param>
        public static void DvlaLogEntry(string errorCode,string errorDetails, string companyId, string source, cEmailTemplates emailTemplates, Employee employee)
        {
            ...
            if (errorCode != null && (errorCode == "504" || errorCode == "506" || errorCode == "501"))
            {
                ...
                emailTemplates.SendMessage(new Guid("9354DBDB-3F77-40CD-843E-02E23F9F23F5"), employee.EmployeeID, recipients);
                if (errorCode== "501")
                {
                    emailTemplates.SendMessage(new Guid("877D893C-3FCB-40A3-A3DD-B2382D79459C"),employee.EmployeeID, recipients);
                }
            }
            else
            {
