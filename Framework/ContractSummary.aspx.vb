...
#End Region
        ...
        Private Sub RenderUFields()
            ...
            Select Case CType(Session("CurTab"), SummaryTabs)
                Case SummaryTabs.ContractDetail
                    ...
                    ufields.createFieldPanel(phCDUFields, cdTable.UserdefinedTable, "cddetails", jscript, Nothing, groupType:=GroupingOutputType.UnGroupedOnly)
                ...
                Case SummaryTabs.ContractAdditional
                    ...
                    If contrCatID = 0 Then
                        ufields.createFieldPanel(phCAUFields, cdTable.UserdefinedTable, "cadetails", jscript, Nothing, groupType:=GroupingOutputType.GroupedOnly)
                    Else
                        ufields.createFieldPanel(phCAUFields, cdTable.UserdefinedTable, "cadetails", jscript, contrCatID, groupType:=GroupingOutputType.GroupedOnly)
                    End If
                ...
                Case SummaryTabs.ContractProduct
                    ...
                    If contrCatID = 0 Then
                        ufields.createFieldPanel(phCPUFields, cpTable.UserdefinedTable, "cpdetails", jscript, Nothing)
                    Else
                        ufields.createFieldPanel(phCPUFields, cpTable.UserdefinedTable, "cpdetails", jscript, contrCatID)
                    End If
            ...
            If jscript.Length > 0 Then
                ClientScript.RegisterClientScriptBlock(Page.GetType, "udfscript_" & Session("CurTab"), "<script language=""javascript"" type=""text\javascript"">eval('" & jscript.ToString & "');</script>")
            End If
        End Sub
        ...
        Private Sub SetViewPermissions(ByVal ActiveViewTab As SummaryTabs)
            ...
            litHelp.Text = "<a href=""./help_text/default_csh.htm" & helpID & """ target=""_blank"" class=""submenuitem"">Help</a>"
        End Sub
        ...
        Private Sub ContractSummary_Load(ByVal ActiveTab As SummaryTabs)
            ...
            If Me.IsPostBack = False Then
                ...
                If Not Request.QueryString("id") Is Nothing Then
                    ...
                    If ContractRoutines.CheckAudienceAccess(curUser.AccountID, curUser.Employee, cAccounts.getConnectionString(curUser.AccountID), "contract_audience", cId) = False Then
                        ' permission denied
                        FWDb.DBClose()
...
#Region "Contract Details Code"
        Private Sub ContractDetails_Load()
            ...
            If Session("ActiveContract") > 0 Then
                ...
                While cacheEnum.MoveNext = True
                    If cacheEnum.Key = "CD" & Session("ActiveContract") Then
                        Dim tmpUserInfo As UserInfo
                        tmpUserInfo = cacheEnum.Value
                        cacheHit = True
                        If tmpUserInfo.UserId <> curUser.EmployeeID Then
                            IsLocked = True
                            LockedBy = tmpUserInfo.FullName
                        Else
                            ' force a cache hit to prevent incorrect expiry
                            tmpUserInfo = Cache("CD" & Session("ActiveContract"))
                        End If
                        Exit While
                    End If
                End While
                ...
                schedSQL.Append("SELECT dbo.CheckContractAccess(@userId,[link_matrix].[contractId]) AS [PermitVal],[link_matrix].[contractId],[contract_details].[contractKey],ISNULL([contract_details].[scheduleNumber],'n/a') AS [scheduleNumber] FROM [link_matrix] " & vbNewLine)
            ...
            If cmdCDUpdate.Visible = False Then
                ...
                cmdCDCancel.ImageUrl = "./buttons/page_close.gif"
            End If
        ...
        Private Sub LoadCDUFData(ByVal fws As cFWSettings, ByVal curUser As CurrentUser, ByVal db As cFWDBConnection, ByVal curContractId As Integer)
            ...
            ufields.populateRecordDetails(phCDUFields, cdTable.UserdefinedTable, ufields.GetRecord(cdTable.UserdefinedTable, curContractId, tables, fields), GroupingOutputType.UnGroupedOnly)
        End Sub
        ...
        Private Function GetContractData(ByVal id As Integer) As Boolean
            ...
            If db.FWDbFlag = True Then
                ...
                '    If Not lstVendor.Items.FindByValue(oldIdx.ToString()) Is Nothing Then
                '        lstVendor.Items.FindByValue(oldIdx).Selected = False
                '    End If
                'End If
                ...
                If newIdx > 0 Then
                    ...
                    litVendorLink.Text = "<a onmouseover=""window.status='Open " & tmpSupplierStr.Trim & " Details screen for this contract " & tmpSupplierStr.ToLower & "';return true;"" onmouseout=""window.status='Done';"" href=""shared/supplier_details.aspx?redir=1&sid=" & lstVendor.SelectedItem.Value.Trim & """><img src=""shared/images/icons/view.png"" alt=""Go To"" /></a>"
                End If
                ...
                Else
                    ...
                    Dim emp As cEmployee = clsEmps.GetEmployeeById(newIdx)
                ...
                If params.EnableFlashingNotesIcon = True Then
                ...
                If curUser.CheckAccessRole(AccessRoleType.View, SpendManagementElement.ContractVariations, True) Then
                    ...
                    ElseIf db.GetFieldValue(db.glDBWorkA, "IsVariation", 0, 0) = "1" Then
                        ...
                        Dim tmpSupplierStr As String = params.SupplierVariationTitle
                        litVendorLink.Text = "<a onmouseover=""window.status='Open " & tmpSupplierStr.Trim & " Details screen for this contract supplier';return true;"" onmouseout=""window.status='Done';"" href=""shared/supplier_details.aspx?redir=1&sid=" & lstVendor.SelectedItem.Value.Trim & """>" & tmpSupplierStr.Trim & "</a>"
                        ...
                        If db.GetRowCount(db.glDBWorkA, 0) > 0 Then
                            ...
                            For Each drow In db.glDBWorkA.Tables(0).Rows
                                ...
                                Select Case CType(drow.Item("AttachmentType"), AttachmentType)
                                    ...
                                    Case Else
                                        strHTML.Append("<option value=""" & Trim(drow.Item("AttachmentType")) & drow.Item("AttachmentId") & """>" & tmpStr & "</option>" & vbNewLine)
                                End Select
        ...
        Public Shared Function DeleteContract(ByVal id As Integer) As String
            ...
            Dim curUser As CurrentUser = cMisc.getCurrentUser(appinfo.User.Identity.Name)
            Dim subaccs As New cAccountSubAccounts(curUser.Account.accountid)
            Dim fws As cFWSettings = cMigration.ConvertToFWSettings(curUser.Account, subaccs.getSubAccountsCollection, curUser.CurrentSubAccountId)
            db.DBOpen(fws, False)
        ...
        Private Sub UpdateContract()
            ...
            If Session("ActiveContract") = 0 Then
                ...
                FWDb.SetFieldValue("lastChangedBy", curUser.Employee.firstname & " " & curUser.Employee.surname, "S", False)
                ...
                If params.ContractNumGen Then
                    ...
                    accProperties.SaveAccountProperties(params, curUser.EmployeeID)
                End If
            ...
            Dim newrec As SortedList(Of Integer, Object) = ufields.getItemsFromPanel(phCDUFields, cdTable.UserdefinedTable, groupType:=GroupingOutputType.UnGroupedOnly)
            ufields.SaveValues(cdTable.UserdefinedTable, Session("ActiveContract"), newrec, tables, fields, curUser, groupType:=GroupingOutputType.UnGroupedOnly)
        ...
        Private Function GetVariationList(ByVal db As cFWDBConnection, ByVal curUser As CurrentUser, ByVal cId As Integer) As String
            ...
            With strHTML
                ...
                If curUser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.ContractVariations, True) Then
                    .Append("<th><img src=""./icons/16/plain/form_green.gif"" alt=""Open Variation"" /></th>" & vbNewLine)
                End If

                'If curUser.CheckAccessRole(AccessRoleType.Delete, SpendManagementElement.ContractVariations, True) Then
                .Append("<th><img src=""./icons/16/plain/folder_lock.gif"" alt=""Close/Reinstate variation"" /></th>" & vbNewLine)
                'End If
                ...
                For Each drow In db.glDBWorkL.Tables(0).Rows
                    ...
                    .Append("<tr>" & vbNewLine)
                    If curUser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.ContractVariations, True) Then
                        .Append("<td class=""" & rowClass & """><a href=""ContractSummary.aspx?tab=" & SummaryTabs.ContractDetail & "&id=" & drow.Item("contractId") & """ title=""Open contract variation"" onmouseover=""window.status='Open contract variation';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/16/plain/form_green.gif"" alt=""Open Variation"" /></a></td>" & vbNewLine)
                    End If

                    If IsDBNull(drow.Item("reviewComplete")) = True Then
                        ' variation is open. provide icon to close
                        .Append("<td class=""" & rowClass & """><a onmouseover=""window.status='Close the variation';return true;"" onmouseout=""window.status='Done';"" href=""ContractSummary.aspx?tab=" & SummaryTabs.ContractDetail & "&id=" & Session("ActiveContract") & "&cdaction=vclose&expand=1&vid=" & drow.Item("contractId") & """ title=""Close the variation""><img src=""./icons/16/plain/folder_lock.gif"" alt=""Close"" /></a></td>" & vbNewLine)
                    Else
                        ' variation is closed. provide icon to re-open
                        .Append("<td class=""" & rowClass & """><a onmouseover=""window.status='Reinstate the variation';return true;"" onmouseout=""window.status='Done';"" href=""ContractSummary.aspx?tab=" & SummaryTabs.ContractDetail & "&id=" & Session("ActiveContract") & "&cdaction=vopen&expand=1&vid=" & drow.Item("contractId") & """ title=""Reinstate the variation""><img src=""./icons/16/plain/folder_out.gif"" alt=""Reinstate"" /></a></td>" & vbNewLine)
                    End If
                    ...
                    Else
                        currencyStr = "N/A"
                    End If
                    .Append("<td class=""" & rowClass & """>" & currencyStr & "</td>" & vbNewLine)
                    .Append("</tr>" & vbNewLine)
                Next
...
#Region "Contract Additional Code"
        Private Sub ContractAdditional_Load()
            ...
            While cacheEnum.MoveNext = True
                If cacheEnum.Key = "CA" & Session("ActiveContract") Then
                    ...
                    Else
                        ' force a cache hit to prevent incorrect expiry
                        tmpUserInfo = Cache("CA" & Session("ActiveContract"))
                    End If
                    Exit While
                End If
            End While
            ...
            If Session("ActiveContract") > 0 Then
                ...
                ufields.populateRecordDetails(phCAUFields, cdtable.UserdefinedTable, ufields.GetRecord(cdtable.UserdefinedTable, Session("ActiveContract"), tables, fields), groupType:=GroupingOutputType.GroupedOnly)
            End If
            ...
            If cmdCAUpdate.Visible = False Then
                Master.useCloseNavigationMsg = True
                'Master.enablenavigation = False
                cmdCACancel.ImageUrl = "./buttons/page_close.gif"
        ...
        Private Sub Update_ContractAdditional()
            ...
            Dim fields As New cFields(curUser.AccountID)
            Dim cdTable As cTable = tables.getTableByName("contract_details")
            Dim cdUFields As List(Of cUserDefinedField) = ufields.GetFieldsByTable(cdTable.UserdefinedTable)

            ' update the user defined fields
            Dim newrec As SortedList(Of Integer, Object) = ufields.getItemsFromPanel(phCAUFields, cdTable.UserdefinedTable, groupType:=GroupingOutputType.GroupedOnly)
            ufields.SaveValues(cdTable.UserdefinedTable, Session("ActiveContract"), newrec, tables, fields, curUser, groupType:=GroupingOutputType.GroupedOnly)
        End Sub
...
#Region "Contract Product Code"
        Private Sub ContractProduct_Load()
            ...
            If cmdCPUpdate.Visible = False Then
                cmdCPCancel.ImageUrl = "~/buttons/page_close.gif"
            Else
        ...
        Private Sub UpdateCPRecord()
            ...
            If curUser.CheckAccessRole(AccessRoleType.View, SpendManagementElement.ViewFinancials, True) Then
                ...
                Else
                    If hasQty And hasMaintVal Then
                        ...
                        tmpVal = Math.Round((tmpMV / tmpQ), 2)
                        dbParams.Add("unitCost", tmpVal)
                        firstEntry = False
                    Else
                        If tmpVal <> "" Then
            ...
            Dim fields As New cFields(curUser.AccountID)
            Dim cpTable As cTable = tables.getTableByName("contract_productdetails")
            Dim cpUFields As List(Of cUserDefinedField) = ufields.GetFieldsByTable(cpTable.UserdefinedTable)
            ...
            Dim newrec As SortedList(Of Integer, Object) = ufields.getItemsFromPanel(phCPUFields, cpTable.UserdefinedTable)
            ufields.SaveValues(cpTable.UserdefinedTable, Session("ActiveContractProduct"), newrec, tables, fields, curUser)
        ...
        Private Sub GetContractProducts(ByVal db As cFWDBConnection, Optional ByVal UFieldlist As String = "")
            ...
            CPHCell = New TableHeaderCell
            CPHImg = New System.Web.UI.WebControls.Image
            CPHImg.ImageUrl = "~/icons/16/plain/document_attachment.gif"
            CPHCell.Controls.Add(CPHImg)
            CPHRow.Cells.Add(CPHCell)

            CPHCell = New TableHeaderCell
            CPHImg = New System.Web.UI.WebControls.Image
            CPHImg.ImageUrl = "~/icons/16/plain/clipboard.gif"
            CPHCell.Controls.Add(CPHImg)
            CPHRow.Cells.Add(CPHCell)

            CPHCell = New TableHeaderCell
            CPHImg = New System.Web.UI.WebControls.Image
            CPHImg.ImageUrl = "~/icons/16/plain/preferences.gif"
            CPHCell.Controls.Add(CPHImg)
            CPHRow.Cells.Add(CPHCell)
            ...
            With CP_PageClose
                .ImageUrl = "./buttons/page_close.gif"
        ...
        Private Sub LoadCPUFData(ByVal contractId As Integer, ByVal conProdId As Integer)
            ...
            Dim fields As New cFields(curUser.AccountID)
            Dim cpTable As cTable = tables.getTableByName("contract_productdetails")
            Dim cdUFields As List(Of cUserDefinedField) = ufields.GetFieldsByTable(cpTable.UserdefinedTable)
            Dim record As SortedList(Of Integer, Object) = ufields.GetRecord(cpTable.UserdefinedTable, conProdId, tables, fields)
            ufields.populateRecordDetails(phCPUFields, cpTable.UserdefinedTable, record)
        End Sub
            ...
            For Each ufrow In dset.Tables(0).Rows
                ...
                Select Case CType(ufrow.Item("Field Type"), UserFieldType)
                    ...
                    Case UserFieldType.StaffName_Ref
                        ...
                        If IsDBNull(drow.Item(ufieldname)) = False Then
                            Dim emps As New cEmployees(curUser.AccountID)
                            Dim emp As cEmployee = emps.GetEmployeeById(drow.Item(ufieldname))

                            If Not emp Is Nothing Then
                                tmpStr = emp.firstname & " " & emp.surname
                            Else
                                tmpStr = "&nbsp;"
                            End If
...
#Region "Invoice Details Code"
        ...
        Private Function GetInvoiceTable(ByVal curUser As CurrentUser, ByVal dset As DataSet, ByVal currencyId As Integer) As String
            ...
            With strHTML
                ...
                .Append("<th><img src=""./icons/16/plain/history.gif"" /></th>" & vbNewLine)
                ...
                For Each drow In dset.Tables(0).Rows
                    ...
                    If curUser.CheckAccessRole(AccessRoleType.View, SpendManagementElement.InvoiceNotes, False) Then
                        .Append("<a href=""ContractSummary.aspx?idaction=notes&tab=" & SummaryTabs.InvoiceDetail & "&invid=" & drow.Item("invoiceId") & "&invnum=" & drow.Item("invoiceNumber") & """ onmouseover=""window.status='Open free text notes for this invoice';return true;"" onmouseout=""window.status='Done';"" title=""Open free text notes for this invoice""><img src=""./icons/16/plain/document_attachment.gif"" /></a>")
                    End If
                    .Append("</td>" & vbNewLine)
                    .Append("<td class=""" & rowClass & """><a href=""ViewStatusHistory.aspx?id=" & drow.Item("invoiceId") & "&desc=" & drow.Item("invoiceNumber") & """ onmouseover=""window.status='View the status history for this invoice';return true;"" onmouseout=""window.status='Done';"" title=""View the status history for this invoice""><img src=""./icons/16/plain/history.gif"" /></a></td>" & vbNewLine)
...
#Region "Note Summary Code"
        ...
        Private Function CreateNotesTable(ByVal db As cFWDBConnection, ByVal notetype As String, ByVal RecId As Integer) As String
            ...
            Select Case LCase(notetype)
                Case "suppliers"
                    strHTML.Append("<div class=""inputpanel"">" & vbNewLine)
        ...
        Private Function GetNoteData(ByVal db As cFWDBConnection, ByVal NoteType As String, ByVal NoteTable As String, ByVal IDField As String, ByVal RecId As Integer, Optional ByVal OwnerId As Integer = 0) As String
            ...
            strNoteTable.Append("<th><img src=""./icons/16/plain/zoom_in.gif"" /></th>" & vbNewLine)
            ...
            For Each drow In db.glDBWorkD.Tables(0).Rows
                ...
                strNoteTable.Append("<td class=""" & rowClass & """><a href=""ViewSingleNote.aspx?id=" & Trim(drow.Item("NoteId")) & "&notetype=" & Trim(NoteType) & "&ownerid=" & RecId.ToString & """><img src=""./icons/16/plain/zoom_in.gif"" /></a></td>" & vbNewLine)
...
#Region "Linked Contracts Code"
        Private Sub LinkedContracts_Load()
            ...
            Else
                litLinkData.Text = "<div class=""inputpanel"" style=""height: 10px;"">Currently no contract links in place</div>"
            End If
        ...
        Private Function GetContractLinks(ByVal db As cFWDBConnection, ByVal cId As Integer) As String
            ...
            headerHTML.Append("<th>&nbsp;</th>" & vbNewLine)
            ...
            For Each drow_link In db.glDBWorkB.Tables(0).Rows
                ...
                For Each drow In db.glDBWorkA.Tables(0).Rows
                    ...
                    If Val(drow.Item("ContractId")) = cId Then
                        tmpStr.Append("<td class=""" & rowClass & """><img src=""./images/arrow_right.gif"" /></td>" & vbNewLine)
                    Else
