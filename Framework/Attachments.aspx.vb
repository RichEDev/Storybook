...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
			...
			If Me.IsPostBack = False Then
				...
				litHelp.Text = "<a href=""./help_text/default_csh.htm#1012"" target=""_blank"" class=""submenuitem"">Help</a>"
			End If
		...
		Private Sub AddAttachment(ByVal refNum As Integer, ByVal att_Area As AttachmentArea)
			...
			If cDef.DBVERSION >= 18 Then
                db.SetFieldValue("AttachedBy", curUser.Employee.firstname & " " & curUser.Employee.surname, "S", False)
			End If
		...
		Private Function GetSimpleAttachmentData(ByVal db As cFWDBConnection) As String
			...
			strHTML.Append("<th><img src=""./icons/16/plain/folder_out.gif"" alt=""Open"" /></th>" & vbNewLine)
			...
			strHTML.Append("<th><img src=""./icons/16/plain/view.gif"" alt=""Audience"" /></th>" & vbNewLine)
				...
				For Each drow In db.glDBWorkA.Tables(0).Rows
					If drow.Item("PermitVal") > 0 Then
                        ...
                        Select Case CType(drow.Item("AttachmentType"), AttachmentType)
                            Case AttachmentType.Secure
                                strHTML.Append("<td class=""" & rowClass & """><a href=""Attachments.aspx?action=email&attarea=" & Trim(att_area.Text) & "&ref=" & Trim(Session("RefNum")) & "&id=" & Trim(drow.Item("AttachmentId")) & """ title=""Open attachment"" onmouseover=""window.status='Open attachment in new window';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/16/plain/folder_out.gif"" alt=""Open"" /></a></td>" & vbNewLine)
                            Case AttachmentType.Audience
                                strHTML.Append("<td class=""" & rowClass & """><a target=""_blank"" href=""ViewAttachment.aspx?id=" & CStr(drow.Item("AttachmentId")) & """ onmouseover=""window.status='Open the attachment in a new window';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/16/plain/folder_out.gif"" alt=""Open"" /></a></td>" & vbNewLine)
                            Case AttachmentType.Hyperlink
                                strHTML.Append("<td class=""" & rowClass & """><a target=""_blank"" href=""" & drow.Item("Directory") & """ onmouseover=""window.status='Open hyperlink document';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/16/plain/folder_out.gif"" alt=""Open"" /></a></td>" & vbNewLine)
                            Case Else
                                strHTML.Append("<td class=""" & rowClass & """><a onclick=""javascript:window.open('ViewAttachment.aspx?id=" & CStr(drow.Item("AttachmentId")) & "');"" onmouseover=""window.status='Open the attachment in a new window';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/16/plain/folder_out.gif"" alt=""Open"" /></a></td>" & vbNewLine)
                        End Select

						If curUser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.Attachments, False) Then
                            strHTML.Append("<td class=""" & rowClass & """><a href=""Attachments.aspx?action=edit&attarea=" & att_area.Text.Trim & "&ref=" & Session("RefNum") & "&id=" & Trim(drow.Item("AttachmentId")) & """ title=""Edit attachment definition"" onmouseover=""window.status='Edit the attachment definition';return true;"" onmouseout=""window.status='Done';""><img src=""./icons/edit.gif"" alt=""Edit"" /></a></td>" & vbNewLine)
						Else
						...
						If curUser.CheckAccessRole(AccessRoleType.Delete, SpendManagementElement.Attachments, False) Then
                            strHTML.Append("<td class=""" & rowClass & """><a href=""javascript:if(confirm('Are you sure you want to delete attachment? OK to confirm.')){window.location.href='Attachments.aspx?action=delete&attarea=" & Trim(att_area.Text) & "&id=" & Trim(drow.Item("AttachmentId")) & "&ref=" & Trim(Session("RefNum")) & "';}""><img src=""./icons/delete2.gif"" /></a></td>" & vbNewLine)
						Else
							strHTML.Append("<td class=""" & rowClass & """>&nbsp;</td>" & vbNewLine)
						End If
                        ...
                        If (drow.Item("PermitVal") > 0 Or drow.Item("permitVal") = 999) And (CType(drow.Item("AttachmentType"), AttachmentType) = AttachmentType.Audience Or CType(drow.Item("AttachmentType"), AttachmentType) = AttachmentType.Hyperlink) And curUser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.AttachmentAudience, False) Then
                            ...
                            strHTML.Append("<td class=""" & rowClass & """><a href=""SetAudience.aspx?type=attachment&id=" & drow.Item("AttachmentId") & """ title=""Update the permitted Audience""><img src=""./icons/16/plain/view.gif"" alt=""Amend"" /></a></td>" & vbNewLine)
                        Else
                            strHTML.Append("<td class=""" & rowClass & """>&nbsp;</td>" & vbNewLine)
                        End If
		...
		Protected Sub lnkLink_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkLink.Click
			...
			litHelp.Text = "<a href=""./help_text/default_csh.htm#" & helpID.ToString & """ target=""_blank"" class=""submenuitem"">Help</a>"
