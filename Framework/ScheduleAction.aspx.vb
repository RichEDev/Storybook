...
Namespace Framework2006
    Partial Class ScheduleAction
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            sql.Append("AND dbo.CheckContractAccess(@userId,[link_matrix].[ContractId]) > 0")
        ...
        Private Function GetSchedules(ByVal db As cFWDBConnection, ByVal curUser As CurrentUser, ByVal contractId As Integer) As String
            ...
            sql.Append("SELECT dbo.CheckContractAccess(@userId,[link_matrix].[ContractId]) AS [PermitVal],[link_matrix].[LinkId],[link_matrix].[ContractId],[contract_details].[ContractDescription],[contract_details].[ContractKey],ISNULL([contract_details].[ScheduleNumber],'') AS [ScheduleNumber],ISNULL([contract_details].[EndDate],'') AS [EndDate],ISNULL([contract_details].[StartDate],'') AS [StartDate],ISNULL([codes_contractstatus].[StatusDescription],'') AS [ContractStatus] FROM [link_matrix] " & vbNewLine)
            ...
            strHTML.Append("<th></th>" & vbNewLine)
            ...
            If db.glNumRowsReturned > 0 Then
                ...
                For Each drow In db.glDBWorkD.Tables(0).Rows
                    ...
                    If drow.Item("ContractId") = curId Then
                        strHTML.Append("<td class=""" & rowClass & """><img src=""./images/arrow_right.gif"" /></td>" & vbNewLine)
                    Else
