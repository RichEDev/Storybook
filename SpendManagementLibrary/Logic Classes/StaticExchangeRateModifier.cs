namespace SpendManagementLibrary.Logic_Classes
{
    ...
    /// <summary>
    /// Populate the Exchange Rate Date Ranges from the Static exchange rate info when turning on <see cref="cAccountProperties.EnableAutoUpdateOfExchangeRates"/>
    /// </summary>

    internal class StaticExchangeRateModifier : ExchangeRateModifier
    {
        public StaticExchangeRateModifier(int accountId) : base(accountId)
        {
        }

        /// <summary>
        /// Populate the Exchange Rate Date Range info based on the current static exchange rates.
        /// </summary>
        ...
        public override void PopulateRanges(DateTime? activatedDate)
        {
            using (var expdata = new DatabaseConnection(cAccounts.getConnectionString(this.AccountId)))
            {
                ...
                expdata.ExecuteProc("dbo.PopulateStaticRanges");
            }
        }
    }
}