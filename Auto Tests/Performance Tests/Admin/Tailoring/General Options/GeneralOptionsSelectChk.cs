...
namespace Auto_Tests
{
    ...
    public class GeneralOptionsSelectChk : WebTest
    {

        public GeneralOptionsSelectChk()
        {
        ...
        public override IEnumerator<WebTestRequest> GetRequestEnumerator()
        {
            ...
            while (reader.Read())
            {
                ...
                // Only test CHECKBOXES remain TICKED
                if (formName.Contains("chk") == true)
                {
                    // Change all of the values to TICKED and validate
                    foreach (WebTestRequest r in IncludeWebTest(new Auto_Tests.GeneralOptionsEdit(formName, "1", "chkon"), false)) { yield return r; }
                }
            }

            // Run a test to check that all tick boxes have remained ticked? May be the case that they can stay ticked individually,
            // but not when other values are ticked aswell


            //foreach (WebTestRequest r in IncludeWebTest(new Auto_Tests.GeneralOptionsEdit(, "1", "chkon"), false)) { yield return r; }
