...
(function () {
    ...
    function TestClientViewModel(data) {
        ...
        if (data.UriPathTemplate.indexOf('/Find?') > -1) {
            data.UriParameters = new Array();
            var path = data.UriPathTemplate.substr(data.UriPathTemplate.indexOf('?') + 1);
            var qryParts = path.split('&');
            for (var j = 0; j < qryParts.length; j++) {
                var qry = qryParts[j].split('=');
                var param = { name: qry[0], value: qry[1], enabled: true };
                var paramVal = ko.observable(param.value);
                var paramEnabled = ko.observable(true);
                data.UriParameters.push(param);
            }
        }
        ...
        addOrReplaceHeader(self.RequestHeaders, "AuthToken", "");
        addOrReplaceHeader(self.RequestHeaders, "version", version);
        ...
        /*  Note - keeping this in just in case it is needed for future.        
