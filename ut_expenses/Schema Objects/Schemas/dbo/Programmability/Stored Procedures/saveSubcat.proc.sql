...
@employeeID INT,
@delegateID INT,
@HomeToOfficeFixedMiles float,
@publicTransportRate int,
@enableDutyOfCare bit,
@requireClassOneBusinessInsurance bit,
@enforceToOfficeMileageCap bit,
@homeToOfficeMileageCap float
AS
declare @id int;
declare @count int
if (@subcatid = 0)
...
begin
 ...
 declare @oldpassengernamesapp bit; 
...
declare @oldEnforceToOfficeMileageCap bit;
 ...
 update subcats set categoryid = @categoryid, pdcatid = @pdcatid, subcat = @subcat, accountcode = @accountcode, description = @description, calculation = @calculation, addasnet = @addasnet, allowanceamount = @allowanceamount, mileageapp = @mileageapp, staffapp = @staffapp, othersapp = @othersapp, attendeesapp = @attendeesapp, pmilesapp = @pmilesapp, bmilesapp = @bmilesapp, tipapp = @tipapp, eventinhomeapp = @eventinhomeapp, receiptapp = @receiptapp, splitentertainment = @splitentertainment, entertainmentid = @entertainmentid, reimbursable = @reimbursable, nonightsapp = @nonightsapp, nodirectorsapp = @nodirectorsapp
   , passengersapp = @passengersapp, nopassengersapp = @nopassengersapp, passengernamesapp = @passengernamesapp, comment = @comment, attendeesmand = @attendeesmand, hotelapp = @hotelapp, noroomsapp = @noroomsapp, hotelmand = @hotelmand, vatnumberapp = @vatnumberapp, vatnumbermand = @vatnumbermand, nopersonalguestsapp = @nopersonalguestsapp, noremoteworkersapp = @noremoteworkersapp, alternateaccountcode = @alternateaccountcode, splitpersonal = @splitpersonal, splitremote = @splitremote, personalid = @personalid, remoteid = @remoteid, reasonapp = @reasonapp, otherdetailsapp = @otherdetailsapp, modifiedon = @date, modifiedby = @userid, shortsubcat = @shortsubcat, fromapp = @fromapp, toapp = @toapp, companyapp = @companyapp, enableHomeToLocationMileage = @enableHomeToLocationMileage, homeToLocationType = @homeToLocationType, mileageCategory = @mileageCategory, isRelocationMileage = @isRelocationMileage, reimbursableSubcatID = @reimbursableSubcatID, allowHeavyBulkyMileage = @allowHeavyBulkyMileage, homeToOfficeAlwaysZero = @homeToOfficeAlwaysZero, StartDate = @startDate, EndDate = @endDate, Validate = @validate,
   HomeToOfficeFixedMiles = @HomeToOfficeFixedMiles, PublicTransportRate = @publicTransportRate,
   EnableDutyOfCare=@enableDutyOfCare,RequireClassOneBusinessInsurance=@requireClassOneBusinessInsurance,
   EnforceHomeToOfficeMileageCap=@enforceToOfficeMileageCap,HomeToOfficeMileageCap=@homeToOfficeMileageCap
 ...
 if @oldpassengernamesapp <> @passengernamesapp
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, null, @oldpassengernamesapp, @passengersapp, @subcat, null;
 ...
 if @oldHomeToOfficeAlwaysZero <> @homeToOfficeAlwaysZero
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, '67F5BC3A-8946-4140-A53B-CD91CB81CBB3', @oldHomeToOfficeAlwaysZero, @homeToOfficeAlwaysZero, @subcat, null;
 if @oldStartDate <> @startDate
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, 'BBEEDD4D-F96B-4EC7-AB90-62CC7544D3E4', @oldStartDate, @startDate, @subcat, null;
 if @oldEndDate <> @endDate
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, '288CD3AF-11C6-40E3-88C5-3FCC9E764A70', @oldEndDate, @endDate, @subcat, null; 
 if @oldValidate <> @validate
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, '2331D55D-3767-4F84-AF7D-11826509FF04', @oldValidate, @validate, @subcat, null; 
 if @oldHomeToOfficeFixedMiles <> @HomeToOfficeFixedMiles
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, null, @oldHomeToOfficeFixedMiles, @HomeToOfficeFixedMiles, @subcat, null; 
 if @oldPublicTransportRate <> @publicTransportRate 
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, null, @oldPublicTransportRate, @publicTransportRate, @subcat, null; 
  if @oldEnableDutyOfCare <> @enableDutyOfCare 
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, null, @oldEnableDutyOfCare, @enableDutyOfCare, @subcat, null; 
 if @oldRequireClassOneBusinessInsurance <> @requireClassOneBusinessInsurance 
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, null, @oldRequireClassOneBusinessInsurance, @requireClassOneBusinessInsurance, @subcat, null;  
 if  ISNULL(@oldEnforceToOfficeMileageCap,0) <>  ISNULL(@enforceToOfficeMileageCap,0) 
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, '0B6B3F0D-B06D-428A-83FD-B36A835D1832', @oldEnforceToOfficeMileageCap, @enforceToOfficeMileageCap, @subcat, null; 
 if ISNULL(@oldHomeToOfficeMileageCap,0) <>  ISNULL(@homeToOfficeMileageCap,0)
  exec addUpdateEntryToAuditLog @employeeID, @delegateID, 29, @id, 'A1EFB523-530C-4BDD-A5A4-03438B683A81', @oldHomeToOfficeMileageCap, @homeToOfficeMileageCap, @subcat, null; 
 END
end
