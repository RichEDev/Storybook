...
BEGIN
	...
	select @username = username from employees inner join cars on cars.employeeid = employees.employeeid where carid = @carid
	...
	if @odometerID = 0
		BEGIN
			...
			exec addUpdateEntryToAuditLog @CUemployeeID, @CUdelegateID, 13, @odometerID, 'fd75e8f0-0317-4689-a059-77bc66c8a98a', '', @oldReading, @recordtitle, null;
			exec addUpdateEntryToAuditLog @CUemployeeID, @CUdelegateID, 13, @odometerID, 'c063831a-af34-4b0a-af76-0966a750c8c6', '', @newReading, @recordtitle, null;
		END
	else
		BEGIN
