CREATE PROCEDURE [dbo].[GetPreviousClaimsByEmployee] @employeeId INT
AS
BEGIN
	...
	INNER JOIN dbo.claims_base ON dbo.employees.employeeid = dbo.claims_base.employeeid ON dbo.currencies.currencyid = dbo.claims_base.currencyid LEFT OUTER JOIN dbo.savedexpenses ON dbo.claims_base.claimid = dbo.savedexpenses.claimid 
	WHERE (dbo.claims_base.submitted = 1) 
	    AND (claims_base.approved = 1 AND claims_base.paid = 1)
