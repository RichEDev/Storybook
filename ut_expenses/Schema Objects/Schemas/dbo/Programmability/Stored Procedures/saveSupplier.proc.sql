...
begin
	...
	BEGIN
	declare @old_supplier_status_description nvarchar(50);
		select @old_supplier_status_description = description from supplier_status where statusid = @oldstatusid;
	declare @supplier_status_description nvarchar(50);
		select @supplier_status_description = description from supplier_status where statusid = @statusid;
		exec addUpdateEntryToAuditLog @CUemployeeID, @CUdelegateID, 52, @supplierid, '751928a2-59d8-4a1d-b987-03891610a243', @old_supplier_status_description, @supplier_status_description, @suppliername,@subaccountid;
	END
	if @oldcategoryid <> @categoryid
	BEGIN
	declare @old_supplier_category nvarchar(50);
		select @old_supplier_category = description from supplier_categories where categoryid = @oldcategoryid;
	declare @supplier_category nvarchar(50);
		select @supplier_category = description from supplier_categories where categoryid = @categoryid;
		exec addUpdateEntryToAuditLog @CUemployeeID, @CUdelegateID, 52, @supplierid, 'ca9e654c-a5a9-4248-9a13-5e0b9fe6f4ee', @old_supplier_category, @supplier_category, @suppliername,@subaccountid;
	END
