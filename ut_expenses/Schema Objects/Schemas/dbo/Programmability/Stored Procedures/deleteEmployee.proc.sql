...
BEGIN
      ...
      set @count = (select count(costcodeid) from costcodes where OwnerEmployeeId = @employeeid)
      if @count > 0
            return 14;
      ...
      IF EXISTS (SELECT approvalMatrixId FROM approvalMatrices WHERE defaultApproverEmployeeId = @employeeid) OR EXISTS (SELECT approvalMatrixLevelId FROM approvalMatrixLevels WHERE approverEmployeeId = @employeeid)
		RETURN 15;

	  IF EXISTS (SELECT COUNT(entityid) FROM customEntities WHERE OwnerId = @employeeid OR supportContactId = @employeeid)
		RETURN 16;
