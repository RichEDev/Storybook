...
declare @approver VARCHAR(100) 
if exists (select * from accountProperties
    where stringKey = 'dutyOfCareEmailReminderForApproverDays')
    SET @reminderdays = 
(SELECT stringValue FROM accountProperties
    WHERE stringKey = 'dutyOfCareEmailReminderForApproverDays')
if exists (select * from accountProperties
    where stringKey = 'dutyOfCareApprover')
    SET  @approver = (SELECT 
stringValue FROM accountProperties
    where stringKey = 'dutyOfCareApprover')
...
SET @sqlQuery ='
        ...
        INSERT INTO #temp      SELECT c.employeeid AS EmployeeId FROM '+@vehicleDocumentsTableId+' vehicle
        INNER JOIN cars c ON c.carid = vehicle.'+@attVehicle +'  
        WHERE CAST('+@attExpireDate +' AS Date) = '+''''+convert (varchar,@expirydate)+''''+' AND vehicle.'+@attVehicleType+' = (SELECT valueid FROM customEntityAttributeListItems WHERE attributeid = '+@attVehicleTypeValue +' AND item = ''Breakdown Cover'')
		 ...
		 BEGIN
				 ...
				 where cast('+@attDrivingExpireDate +' as Date) = '+''''+convert (varchar,@expirydate)+''''+'  
				and ' + @attLicenceType + ' IN((select valueid from customEntityAttributeListItems where attributeid = ' + @attLicenceTypeId + ' and item = ''Photocard'') , (select valueid from customEntityAttributeListItems where attributeid = ' + @attLicenceTypeId + ' and item = ''Driving Licence (Automatic DVLA Lookup)''))
			END'
exec (@sqlQuery)
