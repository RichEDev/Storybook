...
begin
	...
	if @count = 0
		begin
			set @sql = 'insert into [' + @tablename + '] (parentID, audienceID, canview, canedit, candelete, cacheExpiry) values (@recId, @audienceID, @canview, @canedit, @candelete, getdate()); set @newIdentity = scope_identity();';
			set @sqlParms = '@recId int, @audienceID int, @canview bit, @canedit bit, @candelete bit, @newIdentity int OUTPUT';
			exec sp_executesql @sql, @sqlParms, @recId = @recordId, @audienceID = @audienceId, @canview = @canView, @canedit = @canEdit, @candelete = @canDelete, @newIdentity = @returnID output;

			exec addInsertEntryToAuditLog @CUemployeeID, @CUdelegateID, 101, @returnID, @tablename, null;
		end
	else
	begin
		set @sql = 'update [' + @tablename + '] set canview = @canview, canedit = @canedit, candelete = @candelete, CacheExpiry = getdate() where parentID = @recID and audienceID = @audienceID';
		set @sqlParms = '@recID int, @audienceID int, @canview bit, @canedit bit, @candelete bit';
		exec sp_executesql @sql, @sqlParms, @recID = @recordId, @audienceID = @audienceId, @canview = @canView, @canedit = @canEdit, @candelete = @canDelete;
