...
CREATE PROCEDURE [dbo].[saveESRTrust]
      ...
      @esrTrustVersionNumber tinyint,
      @currentOutboundSequence int,
      @versionNumberChanged tinyint output
AS 
      DECLARE @count INT;

      SET @versionNumberChanged = 0;

      IF (@trustID = 0)
            BEGIN
                  SET @count = (SELECT COUNT(trustID) FROM esrTrusts WHERE trustName = @trustName);
            
                  IF @count > 0
                        RETURN -1;
				  IF @trustVPD > ''
				  BEGIN
					SET @count = (SELECT COUNT(trustID) FROM esrTrusts WHERE trustVPD = @trustVPD);
                  IF @count > 0
                        RETURN -2;
				  END

                  INSERT INTO esrTrusts (trustName, trustVPD, periodType, periodRun, ftpAddress, ftpUsername, ftpPassword, createdOn, runSequenceNumber, delimiterCharacter, ESRVersionNumber, currentOutboundSequence) VALUES (@trustName, @trustVPD, @periodType, @periodRun, @ftpAddress, @ftpUsername, @ftpPassword, getutcdate(), @runSequenceNumber, @delimiterCharacter, @esrTrustVersionNumber, @currentOutboundSequence);
      ...
      ELSE
            BEGIN
			      ...
			      IF @trustVPD > ''
				  BEGIN
					SET @count = (SELECT COUNT(trustID) FROM esrTrusts WHERE trustVPD = @trustVPD and trustID <> @trustID);
                  IF @count > 0
                        RETURN -2;
				  END
                  ...
                  declare @oldCurrentOutboundSequence int;
                  declare @oldESRVersionNumber tinyint;

                  select @oldtrustName = trustName, @oldtrustVPD = trustVPD, @oldperiodType = periodType, @oldperiodRun = periodRun, @oldftpAddress = ftpAddress, @oldftpUsername = ftpUsername, @oldftpPassword = ftpPassword, @oldRunSequence = runSequenceNumber, @oldDelimiterCharacter = delimiterCharacter, @oldESRVersionNumber=ESRVersionNumber, @oldCurrentOutboundSequence = currentOutboundSequence from esrTrusts WHERE trustID=@trustID;

                  UPDATE esrTrusts SET trustName=@trustName, trustVPD=@trustVPD, periodType=@periodType, periodRun=@periodRun, ftpAddress=@ftpAddress, ftpUsername=@ftpUsername, ftpPassword=@ftpPassword, modifiedOn = getutcdate(), runSequenceNumber=@runSequenceNumber, delimiterCharacter = @delimiterCharacter, ESRVersionNumber=@esrTrustVersionNumber, currentOutboundSequence=@currentOutboundSequence WHERE trustID=@trustID;
                  ...
                  if @oldCurrentOutboundSequence <> @currentOutboundSequence
                        exec addUpdateEntryToAuditLog @employeeID, @delegateID, 27, @trustID, '5B6C4743-0D2B-4DD6-B4D4-E7D343A5A962', @oldCurrentOutboundSequence, @currentOutboundSequence, @trustName, null;
                  if @oldESRVersionNumber <> @esrTrustVersionNumber
                  begin
                        exec addUpdateEntryToAuditLog @employeeID, @delegateID, 27, @trustID, '534B3DA5-06E1-40A1-8444-EF37D9858986', @oldESRVersionNumber, @esrTrustVersionNumber, @trustName, null;
                        SET @versionNumberChanged = 1;
                  end
            END
RETURN @trustID;