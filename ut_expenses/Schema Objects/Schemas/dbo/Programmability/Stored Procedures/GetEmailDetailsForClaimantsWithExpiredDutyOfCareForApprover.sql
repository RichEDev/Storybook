...
CREATE PROCEDURE [dbo].[GetEmailDetailsForClaimantsWithExpiredDutyOfCareForApprover] @employeeid INT
AS
BEGIN
	...
	DECLARE @attLicenceType VARCHAR(20) = [dbo].[GetAttributeId]('CD3164BF-5C67-47F9-AF02-2FA6BB6F0BCE', 1) ---attt156e8
	DECLARE @attLicenceTypeValue VARCHAR(20) = [dbo].[GetAttributeId]('CD3164BF-5C67-47F9-AF02-2FA6BB6F0BCE', 0) ---1568
	DECLARE @attLicenceNumber VARCHAR(20) = [dbo].[GetAttributeId]('F8123BD2-0E21-43C3-9325-BE3AF17A95DB', 1) ---attt1563
...
INSERT INTO #temp
SELECT ''Breakdown Cover Document for '' + c.make ' + '+ '' '' +' + '  c.model ' + '+ '' '' +' + '  c.registration AS DocumentName, ' + convert(VARCHAR, @reminderdays) + ' AS NumberOfDays,  cast(' + '''' + convert(VARCHAR, @expirydate) + '''' + ' AS Date) AS ExpiryDate FROM ' + @vehicleDocumentsTableId + ' vehicle
INNER JOIN cars c ON c.carid = vehicle.' + @attVehicle + '  
