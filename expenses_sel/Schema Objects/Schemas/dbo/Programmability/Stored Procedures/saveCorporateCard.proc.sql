...
if (@count = 0)
begin
	insert into corporate_cards ([cardproviderid], claimants_settle_bill, createdby, createdon, allocateditem, blockcash, reconciled_by_admin, singleclaim, blockunmatched) values (@cardproviderid, @claimantsettlesbill, @userid, @date, @allocateditem, @blockcash, @reconciledbyadministrator, @singleclaim, @blockunmatched)
...
begin
	...
	update corporate_cards set [cardproviderid] = @cardproviderid, claimants_settle_bill = @claimantsettlesbill, modifiedby = @userid, modifiedon = @date, allocateditem = @allocateditem, blockcash = @blockcash, reconciled_by_admin = @reconciledbyadministrator, singleclaim = @singleclaim, blockunmatched = @blockunmatched where cardproviderid = @cardproviderid;
