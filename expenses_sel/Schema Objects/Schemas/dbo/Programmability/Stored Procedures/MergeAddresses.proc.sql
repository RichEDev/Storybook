		...
		BEGIN TRANSACTION MergeAddresses
			...
			DECLARE loop_cursor CURSOR FOR
				...
				BEGIN
					...
					--Update company IDs on saved expenses current
					DECLARE curCompany_cursor CURSOR FOR
						SELECT expenseid FROM savedexpenses_current WHERE companyid = @ID
						OPEN curCompany_cursor
						FETCH NEXT FROM curCompany_cursor INTO @expenseid
						WHILE @@FETCH_STATUS = 0
						BEGIN
							UPDATE savedexpenses_current SET companyid=@newCompanyID WHERE expenseid = @expenseid;
					...
					SET @Count = 0;
					--Update company IDs on saved expenses previous
					SET @Count = (SELECT count(expenseid) FROM savedexpenses_previous WHERE companyid = @ID);
					IF @Count > 0
					BEGIN
						DECLARE prevCompany_cursor CURSOR FOR
							SELECT expenseid FROM savedexpenses_previous WHERE companyid = @ID
							OPEN prevCompany_cursor
							FETCH NEXT FROM prevCompany_cursor INTO @expenseid
							WHILE @@FETCH_STATUS = 0
							BEGIN
								UPDATE savedexpenses_previous SET companyid=@newCompanyID WHERE expenseid = @expenseid;
								SET @companyCount = @companyCount + 1;
								...
								SET @username = (SELECT username from employees WHERE employeeid in (SELECT employeeid from claims_base WHERE claimid in (SELECT claimid FROM savedexpenses WHERE expenseid = @expenseid)))
								SET @recordtitle = (SELECT 'Username: ' + @username + ' Claim: ' + @claimName + ' Mileage expense date: ' + CAST(@expenseDate AS nvarchar(30)))
								exec addUpdateEntryToAuditLog @userID, @delegateID, 105, @expenseid, 'a8e49944-9ee5-4936-9401-544a0e8057b6', @oldAddress, @newAddress, @recordTitle, null;
							FETCH NEXT FROM prevCompany_cursor INTO @expenseid
							END
						CLOSE prevCompany_cursor
						DEALLOCATE prevCompany_cursor
						
						--Need to recache any associated claims by updating the modified on date
						--There is no SQL Cache Dependency on savedexpenses_previous so need to 
						--update the claim to show the merged locations
						UPDATE claims_base SET Modifiedon = getutcdate() where claimid in (select claimid from savedexpenses where companyid = @newCompanyID);
					END
