...
AS 
	...
	IF (@teamid = 0)
		BEGIN
			SET @alreadyExists = (SELECT COUNT(*) FROM teams WHERE teamname=@teamname and subAccountId = @subAccountID)
			IF @alreadyExists > 0
				RETURN -1;
	...
	ELSE
		BEGIN
			SET @alreadyExists = (SELECT COUNT(*) FROM teams WHERE teamname=@teamname AND teamid <> @teamid and subAccountId <> @subAccountID)
			IF @alreadyExists > 0
				RETURN -1;
