namespace ApiCrud.DataAccess
{
    ...
    public class ApiDbConnection : IApiDbConnection
    {
        ...
        /// The logger DataBase.
        /// </param>
        public ApiDbConnection(string metabase, int accountId, ApiLibrary.Interfaces.IApiDbConnection loggerDb = null )
        {
        ...
        public int ExecuteProc(string strsql)
        {
            using (var dataconnection = new SqlConnection(this.ConnectionString))
            {
                ...
                {
                    EventLog.WriteEntry("Application", string.Format("Execute Proc error \n {0} \n {1} \n SQL = {2}", ex.Message, ex.StackTrace, strsql));
                    Debug.WriteLine(ex);
                    this.ErrorMessage = ex.Message;

                    var sb = new StringBuilder(); 
                    foreach (SqlParameter parameter in Sqlexecute.Parameters)
                    {
