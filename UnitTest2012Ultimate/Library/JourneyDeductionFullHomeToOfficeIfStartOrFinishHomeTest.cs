...
namespace UnitTest2012Ultimate.Library
{
    ...
    /// </summary>
    [TestClass]
    public class JourneyDeductionFullHomeToOfficeIfStartOrFinishHomeTest
    {
        #region Fields
        ...
        /// No deductions here
        /// </summary>
        [TestMethod, TestCategory("Spend Management Library"), TestCategory("Journey Deduction Rules - Full Home To Office If Start Or Finish Home")]
        ...
        /// </summary>
        [TestMethod, TestCategory("Spend Management Library"), TestCategory("Journey Deduction Rules - Full Home To Office If Start Or Finish Home")]
        public void DeductWhereHomeAddressInMiddleOfJourney()
        {
            var steps = new SortedList<int, cJourneyStep>();
            const decimal Step1EnteredDistance = 32.3m;
            steps.Add(0, new cJourneyStep(1, new Address { Identifier = 11110 }, new Address { Identifier = 11111 }, Step1EnteredDistance, 32.2m, 0, 0, 32.3m, false));
        ...
        public void DeductWhereHomeAddressInFirstJourneyStepAndDeductionDistanceExceedsFirstStepDistance()
        {
            ...
            var expectedStep2Deduction = Step2EnteredDistance - (HomeToOfficeDistance - steps[0].nummiles);

            var deductedSteps = rule.Deduct();

            Assert.AreEqual(deductedSteps[0].nummiles, 0);
            Assert.AreEqual(deductedSteps[1].nummiles, expectedStep2Deduction);
            Assert.AreEqual(deductedSteps[2].nummiles, Step3EnteredDistance);
        }
        ...
        public void DeductWhereHomeAddressInLastJourneyStepAndDeductionDistanceExceedsLastStepDistance()
        {
            ...
            var expectedStep2Deduction = Step2EnteredDistance - (OfficeToHomeDistance - steps[2].nummiles);
        ...
        public void DeductWhereHomeAddressInFirstAndLastJourneyStepAndDeductionDistanceExceedsFirstAndLastStepDistance()
        {
            ...
            var expectedStep2Deduction = Step2EnteredDistance - (HomeToOfficeDistance - steps[0].nummiles);
            var expectedStep3Deduction = Step3EnteredDistance - (OfficeToHomeDistance - steps[3].nummiles);
        ...
        /// <summary>
        /// Deduction from first and last step, deduction distances exceed first step distance, but where last step doesn't end at home.
        /// (i.e. test that it deducts from all steps when it's only traversing one way through them, rather than excluding the last one)
        /// Fix for work item 51825.
        /// </summary>
        [TestMethod]
        public void DeductsFromBothStepsIfDeductionDistanceGreaterThanFirstStepButOnlyFirstStopIsHome()
        {
            var steps = new SortedList<int, cJourneyStep>();

            const decimal mileageFirstStep = 6m;
            const decimal mileageSecondStep = 7m;

            var homeLocation = new Address { Identifier = HomeLocationId};
            var intermediateLocation = new Address { Identifier = 11111};
            var endLocation = new Address { Identifier = 11112 };

            steps.Add(0, new cJourneyStep(1, homeLocation, intermediateLocation, mileageFirstStep, mileageFirstStep, 0, 0, mileageFirstStep, false));
            steps.Add(1, new cJourneyStep(1, intermediateLocation, endLocation, mileageSecondStep, mileageSecondStep, 0, 1, mileageSecondStep, false  ));
            //first step 6, second step 7, total 13.
            //total deductable: 10.2 (HomeToOfficeDistance)
            //should deduct all 6 off first step, and remaining 4.2 off second step, leaving 0 and 2.8.

            var rule = new FullHomeToOfficeIfStartOrFinishHome(steps, HomeToOfficeDistance, OfficeToHomeDistance, HomeLocationId, OfficeLocationId, new cSubcat());
            var deductedSteps = rule.Deduct();
            Assert.AreEqual(0m, deductedSteps[0].nummiles);
            Assert.AreEqual(2.8m, deductedSteps[1].nummiles);
        }


        /// <summary>
        /// Deduction from first and last step, deduction distances exceed first step distance, but where first step doesn't start at home.
        /// (i.e. test that it deducts from all steps when it's only traversing one way through them, rather than excluding the last one)
        /// Fix for work item 51825.
        /// </summary>
        [TestMethod]
        public void DeductsFromBothStepsIfDeductionDistanceGreaterThanFirstStepButOnlyLastStopIsHome()
        {
            var steps = new SortedList<int, cJourneyStep>();
            const decimal mileageFirstStep = 6m;
            const decimal mileageSecondStep = 7m;
            var startLocation = new Address { Identifier = 11110};
            var intermediateLocation = new Address { Identifier = 11111};
            var homeLocation = new Address { Identifier = HomeLocationId };

            steps.Add(0, new cJourneyStep(1, startLocation, intermediateLocation, mileageFirstStep, mileageFirstStep, 0, 0, mileageFirstStep, false));
            steps.Add(1, new cJourneyStep(1, intermediateLocation, homeLocation, mileageSecondStep, mileageSecondStep, 0, 1, mileageSecondStep, false  ));
            //first step 6, second step 7, total 13.
            //total deductable: 10.9 (OfficeToHomeDistance)
            //should deduct all 7 off last step, and remaining 3.9 off second step, leaving 2.1 and 0.

            var rule = new FullHomeToOfficeIfStartOrFinishHome(steps, HomeToOfficeDistance, OfficeToHomeDistance, HomeLocationId, OfficeLocationId, new cSubcat());
            var deductedSteps = rule.Deduct();
            Assert.AreEqual(2.1m, deductedSteps[0].nummiles);
            Assert.AreEqual(0m, deductedSteps[1].nummiles);
        }
