...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            If Me.IsPostBack = False Then
                Try
                    ...
                    curUser.CheckAccessRole(AccessRoleType.View, SpendManagementElement.Emails, False, True)
        ...
        Private Function GetEmailTemplates(ByVal db As cFWDBConnection, ByVal curuser As CurrentUser) As String
            ...
            If curuser.CheckAccessRole(AccessRoleType.Delete, SpendManagementElement.Emails, False) Then
                cols += 1
                strHTML.Append("<th width=""25px""><img src=""./icons/delete2.gif"" /></th>" & vbNewLine)
            End If
            If curuser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.Emails, False) Then
                cols += 1
                strHTML.Append("<th width=""25px""><img src=""./icons/16/plain/document_exchange.png"" /></th>" & vbNewLine)
            End If
            ...
            If db.glNumRowsReturned > 0 Then
                For Each drow In db.glDBWorkA.Tables(0).Rows
                    ...
                    strHTML.Append("<tr>" & vbNewLine)
                    If curuser.CheckAccessRole(AccessRoleType.Delete, SpendManagementElement.Emails, False) Then
                        strHTML.Append("<td class=""" & rowClass & """><a onmouseover=""window.status='Delete this uploaded template?';return true;"" onmouseout=""window.status='Done';"" href=""javascript:if(confirm('Are you sure you want to delete this template?'));{window.location.href='VerifyTemplates.aspx?action=delete&id=" & drow.Item("templateId") & "';}""><img src=""./icons/delete2.gif"" /></a></td>" & vbNewLine)
                    End If
                    If curuser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.Emails, False) Then
                        strHTML.Append("<td class=""" & rowClass & """><a onmouseover=""window.status='Replace Template " & drow.Item("templateName") & "';return true"" onmouseout=""window.status='Done';"" onclick=""javascript:document.location='ImportReportFields.aspx?action=emailtemplate&method=replace&id=" & drow.Item("templateId") & "&tname=" & drow.Item("templateName") & "';""><img src=""./icons/16/plain/document_exchange.png"" /></a></td>" & vbNewLine)
                    End If
            ...
            Else
                strHTML.Append("<tr>" & vbNewLine)
                strHTML.Append("<td class=""row1"" align=""center"" colspan=""" & cols.ToString & """>No Email Templates are currently uploaded</td>" & vbNewLine)
                strHTML.Append("</tr>" & vbNewLine)
            End If
