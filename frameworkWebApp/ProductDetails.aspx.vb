...
#End Region
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Dim udTable As cTable = tables.getTableById(pTable.UserDefinedTableID)
            Dim jscriptBlock As New StringBuilder()

            ufields.createFieldPanel(phPUFields, udTable, "productUFields", jscriptBlock)

            If jscriptBlock.Length > 0 Then
                ScriptManager.RegisterStartupScript(Me, Page.GetType, "udfscript", jscriptBlock.ToString, True)
            End If
        ...
        Private Sub ShowProduct(ByVal FWDb As cFWDBConnection, ByVal ProductID As Integer)
            ...
            If FWDb.FWDbFlag = True Then
                ...
                If ProductID > 0 Then
                    Dim udTable As cTable = tables.getTableById(pTable.UserDefinedTableID)
                    udfs = ufields.GetRecord(udTable, ProductID, tables, fields) ' cUFInterim.GetUFRecordValues(curUser.Account.accountid, ProductID, ufields.getFieldsByTable(ptable))
                    ufields.populateRecordDetails(phPUFields, udTable, udfs)
                Else
                ...
                If curUser.CheckAccessRole(AccessRoleType.View, SpendManagementElement.ProductLicences, False) Then
                    litLicences.Text = "<a href=""contracts/ProductLicences.aspx?pid=" & ProductID.ToString & """ onmouseover=""window.status='View / Modify licences for this product';return true;"" onmouseout=""window.status='Done';"">Licences</a>"
                Else
    ...
    "WHERE [product_suppliers].[productId] = @productId ORDER BY [supplier_details].[suppliername]"
        ...
        Private Sub UpdateProduct()
            ...
            Dim fields As New cFields(fws.MetabaseCustomerId)
            Dim pdTable As cTable = tables.getTableByName("productDetails")
            Dim udTable As cTable = tables.getTableById(pdTable.UserDefinedTableID)
            Dim newrec As SortedList(Of Integer, Object) = ufields.getItemsFromPanel(phPUFields, udTable)

            ufields.SaveValues(udTable, Session("ProductId"), newrec, tables, fields, curUser, elementId:=SpendManagementElement.Products, record:=txtProductName.Text)
        ...
        Private Function GetProducts(ByVal filterStr As String, ByVal filterCategory As Integer) As String
            ...
            With strHTML
                ...
                If curUser.CheckAccessRole(AccessRoleType.Edit, SpendManagementElement.Products, True) Then
                    .Append("<th width=""25px""><img src=""./icons/edit.gif"" /></th>" & vbNewLine)
                End If

                If curUser.CheckAccessRole(AccessRoleType.Delete, SpendManagementElement.Products, False) Then
                    .Append("<th width=""25px""><img src=""./icons/delete2.gif""/></th>" & vbNewLine)
                End If
                    ...
                    Select Case CType(drow.Item("Field Type"), UserFieldType)
                        ...
                        Case UserFieldType.StaffName_Ref
                            ...
                            If employees.getCount(curUser.Account.accountid) > cDef.UF_MAXCOUNT Then
                                ...
                                Dim emp As Employee = employees.GetEmployeeById(selectedIdx)

                                litStr.Append("<input type=""text"" readonly name=""" & tmpStr & "_TXT"" value=""" & emp.Forename + " " + emp.Surname & """/>")
        ...
        Protected Sub lnkLicences_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkLicences.Click
            Response.Redirect("~/contracts/ProductLicences.aspx?pid=" & Session("ProductId"), True)
        End Sub
