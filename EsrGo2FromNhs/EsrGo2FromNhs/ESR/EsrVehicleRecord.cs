namespace EsrGo2FromNhs.ESR
{
    ...
    public class EsrVehicleRecord : EsrVehicle
    {
        ...
        public static EsrVehicleRecord ParseEsrVehicleRecord(string recordLine)
        {
            ...
            DateTime esrLastUpdate = new DateTime(1900, 1, 1, 0, 0, 0);
            decimal enginecc = 0;
            ...
            if (rec.Length != ((int)VehicleRecordColumnsRef.FuelType + 1) && rec.Length != 2) // 2 is delete record length (fixed)
            {
            ...
            switch (rec[(int)VehicleRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrVehicleUpdateRecordType:
                    ...
                    DateTime tmpDate;
                    if (DateTime.TryParseExact(rec[(int)VehicleRecordColumnsRef.EffectiveStartDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                        effectStartDate = tmpDate;
                    }

                    if (DateTime.TryParseExact(rec[(int)VehicleRecordColumnsRef.EffectiveEndDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                    ...
                    if (DateTime.TryParseExact(rec[(int)VehicleRecordColumnsRef.InitialRegistrationDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                        initialRegistrationDate = tmpDate;
                    }
                    if (DateTime.TryParseExact(rec[(int)VehicleRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
