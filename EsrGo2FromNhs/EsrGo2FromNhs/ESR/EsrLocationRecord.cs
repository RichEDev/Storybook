namespace EsrGo2FromNhs.ESR
{
    ...
    public class EsrLocationRecord : EsrLocation
    {
        ...
        public static EsrLocationRecord ParseEsrLocationRecord(string recordLine)
        {
            ...
            if (rec.Length != ((int)LocationRecordColumnsRef.LastUpdateDate + 1) && rec.Length != EsrFile.RecordDeleteLength)
            {
            ...
            switch (rec[(int)LocationRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrLocationUpdateRecordType:
                    ...
                    if (DateTime.TryParseExact(rec[(int)LocationRecordColumnsRef.InactiveDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpInactiveDate))
                    {
                    ...
                    if (DateTime.TryParseExact(rec[(int)LocationRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpLastUpdateDate))
                    {
