namespace EsrGo2FromNhs.ESR
{
    ...
    public class EsrPhoneRecord : EsrPhone
    {
        ...
        public static EsrPhoneRecord ParseEsrPhoneRecord(string recordLine)
        {
            ...
            if (rec.Length != ((int)PhoneRecordColumnsRef.LastUpdateDate + 1) && rec.Length != 2) // 2 is delete record length (fixed)
            {
            ...
            switch (rec[(int)PhoneRecordColumnsRef.RecordType].ToUpper())
            {
                ...
                case EsrRecordTypes.EsrPhoneDeleteRecordType:
                    ...
                    long.TryParse(rec[global::EsrGo2FromNhs.ESR.EsrFile.RecordDeletionColumnRef], out esrPhoneId);
                    break;
                default:
                    return null;
            }
