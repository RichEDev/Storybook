namespace EsrGo2FromNhs.ESR
{
    ...
    public class EsrPositionRecord : EsrPosition
    {
        ...
        public static EsrPositionRecord ParseEsrPositionRecord(string recordLine)
        {
            ...
            DateTime lastUpdateDate = new DateTime(1900, 1, 1);
            ...
            if (rec.Length != ((int)PositionRecordColumnsRef.SubjectiveCodeDescription + 1) && rec.Length != global::EsrGo2FromNhs.ESR.EsrFile.RecordDeleteLength)
            {
            ...
            switch (rec[(int)PositionRecordColumnsRef.RecordType].ToUpper())
            {
                case EsrRecordTypes.EsrPositionUpdateRecordType:
                    ...
                    DateTime tmpDate;
                    if (DateTime.TryParseExact(rec[(int)PositionRecordColumnsRef.EffectiveFromDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                        effectiveFromDate = tmpDate;
                    }

                    if (DateTime.TryParseExact(rec[(int)PositionRecordColumnsRef.EffectiveToDate], "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                    ...
                    if (DateTime.TryParseExact(rec[(int)PositionRecordColumnsRef.LastUpdateDate], "yyyyMMdd HHmmss", CultureInfo.InvariantCulture, DateTimeStyles.None, out tmpDate))
                    {
                ...
                case EsrRecordTypes.EsrPositionDeleteType:
                    ...
                    long.TryParse(rec[global::EsrGo2FromNhs.ESR.EsrFile.RecordDeletionColumnRef], out esrPositionId);
                    break;
                default:
                    return null;
            }
