    ...
    public class EsrRecordProcessor
    {
        ...
        private void UpdateFileSequence()
        {
            ...
            if (this._currentTrust != null)
            {
                ...
                this._currentTrust.currentOutboundSequence = headerRecord.UniqueFileSequenceId;
        ...
        private EsrHubStatus.EsrHubTransferStatus ValidateFile(List<string> fileRows)
        {
            ...
            {
                this._currentTrust = GetVpdAccountId(headerRecord.VpdNumber);
                ...
                if (this._logRecord == null)
                {
                    this._logRecord = new ImportLog { Action = Action.Create, logType = 2, logName = headerRecord.Filename, expectedLines = fileRows.Count };
                    this._logRecord = this.UpdateLog();
                }
                ...
                if (this._currentTrust.currentOutboundSequence.HasValue && ((headerRecord.UniqueFileSequenceId != this._currentTrust.currentOutboundSequence) && (headerRecord.UniqueFileSequenceId != (this._currentTrust.currentOutboundSequence.Value + 1))))
                {
                    this._logger.Write(
                        "expenses",
                        headerRecord.VpdNumber,
        ...
        {
            return this._apiRpc.GetAccountFromVpd(vpdNumber.ToString(CultureInfo.InvariantCulture));
        }
        ...
        {
            var reprocessedRecs = new List<object>();
            var orgRecs = new List<EsrOrganisationRecord>();
            var perRecs = new List<EsrPersonRecord>();
            int recIdx = 0;

            foreach (object record in processedRecords.Where(x => x!= null))
            {
                recIdx++;

                if (record.GetType() == typeof(EsrOrganisationRecord))
                {
                    var organisationRecord = (EsrOrganisationRecord)record; 
                ...
                var personRecord = (EsrPersonRecord)record; 
        ...
        private IEnumerable<EsrLocationRecord> SendLocationRecords(ref List<EsrLocationRecord> locationRecords)
        {
            ...
            try
            {
                outputRecords.AddRange(this._apiRpc.UpdateEsrLocationRecords(this._accountId, headerRecord.VpdNumber.ToString(CultureInfo.InvariantCulture), locationRecords));
        ...
        private IEnumerable<EsrOrganisationRecord> SendOrganisationRecords(ref List<EsrOrganisationRecord> organisationRecords)
        {
            ...
            try
            {
                outputRecords.AddRange(this._apiRpc.UpdateEsrOrganisations(this._accountId, headerRecord.VpdNumber.ToString(CultureInfo.InvariantCulture), organisationRecords));
        ...
        private IEnumerable<EsrPersonRecord> SendPersonRecords(ref List<EsrPersonRecord> personRecords)
        {
            ...
            try
            {
                outputRecords.AddRange(this._apiRpc.UpdateEsrPersonRecords(this._accountId, headerRecord.VpdNumber.ToString(CultureInfo.InvariantCulture), personRecords));
                personRecords.Clear();
            }
            catch (Exception ex)
            {
