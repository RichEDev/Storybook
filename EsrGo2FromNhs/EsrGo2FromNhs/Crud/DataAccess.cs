namespace EsrGo2FromNhs.Crud
{
        ...
        where T : DataClassBase, new()
    {
        ...
        /// <summary>
        /// The key "UseBatchSavesForSingleRecords" in the config file determines whether or not to use the batch stored procedures for saving batches of a singular record
        /// If the key is missing, the value true is assumed.
        /// </summary>
        private readonly bool _useBatchSavesForSingleRecords;
        ...
        public DataAccess(IApiDbConnection dataConnection, IApiDbConnection loggerDb = null)
        {
            ...
            string batchSaveSetting = ConfigurationManager.AppSettings["UseBatchSavesForSingleRecords"] ?? "true";
            this._useBatchSavesForSingleRecords =  String.Compare(batchSaveSetting, "true", StringComparison.OrdinalIgnoreCase) == 0;
        }
        ...
        public List<T> Create(List<T> entities)
        {
            ...
            if (results.Count != 1 || this._useBatchSavesForSingleRecords)
            {
                return this.StoredProcedureSaveBatch(results);
            }

            T returnValue = this.StoredProcedureSave(results.First());
        ...
        protected List<T> StoredProcedureSaveBatch(List<T> entityList)
        {
            ...
            if (table.Rows.Count > 0)
            {
                ...
                {
                    using (var dr = this.Connection.GetStoredProcReader(saveStoredProcedure))
                    {
                        var i = 0;
                        while (dr != null && dr.Read())
                        {
                            returnList[i].KeyValue = (long)dr[returnList[i].KeyField.Name];
                            i++;
                        }
                    }
                }
