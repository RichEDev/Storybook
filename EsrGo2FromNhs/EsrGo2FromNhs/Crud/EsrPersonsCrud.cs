namespace EsrGo2FromNhs.Crud
{
    ...
    public class EsrPersonsCrud : EntityBase, IDataAccess<EsrPersonRecord>
    {
        ...
        public List<EsrPersonRecord> Create(List<EsrPersonRecord> entities)
        {
            ...
            int itemRoleId = int.Parse(this.GetImportFormat("defaultItemRole"));
            int accessRoleId = int.Parse(this.GetImportFormat("defaultRole"));
            ...
            List<EmployeeWorkAddress> allExistingWorkAddresses = this.EsrApiHandler.Execute<EmployeeWorkAddress>(DataAccessMethod.ReadAll);

            IEnumerable<Address> uniqueAddresses = UniqueAddresses(allAddresses);
            ...
            foreach (Address address in uniqueAddresses)
            {
                ...
                foreach (EsrPersonRecord personRecord in person)
                {
                    if (personRecord != null && address.AddressID > 0 && personRecord.EmployeeId > 0)
                    {
                        ...
                        var assignments = personRecord.EsrAssignments.Where(x => x.EsrLocationId == address1.EsrLocationID);
            ...
            if (this.ImportElementType(TemplateMapping.ImportElementType.Location, this.TrustVpd))
            {
                var workResult = Mapper<EmployeeWorkAddress>.SubmitList(workAddresses, this.EsrApiHandler);
            ...
            if (employeesToDelete.Count == 1)
            {
                ...
                if (employeeResult[0].ActionResult.Result == ApiActionResult.Success)
                {
                    ...
                    if (result.Vehicles != null)
                    {
                        ...
                        {
                            var currentCar = this.EsrApiHandler.Execute<Car>(DataAccessMethod.ReadByEsrId, employeeResult[0].employeeid.ToString(CultureInfo.InvariantCulture));
        ...
        private IEnumerable<Car> CreateUpdateCars(EsrPersonRecord personRecord)
        {
            ...
            if (mapping != null)
            {
                ...
                {
                    var currentCarList = this.EsrApiHandler.Execute<Car>(DataAccessMethod.ReadByEsrId, personRecord.EmployeeId.ToString(CultureInfo.InvariantCulture));
                    ...
                    currentCar = this.FindCarByRegistration(
                        vehicle.RegistrationNumber,
                        currentCarList,
                        currentCarAssignments,
                        vehicle.ESRVehicleAllocationId);
        ...
        internal List<EsrAssignment> UpdateAssignmentMapping(EsrPersonRecord esrPerson)
        {
            ...
            if (mapping != null && esrPerson.EsrAssignments != null)
            {
                    ...
                    if (assignment.ESRPersonId != null)
                    {
                        ...
                        if (currentAssignment != null && currentAssignment.Count > 0)
                        {
                            ...
                            foreach (EsrAssignment esrAssignment in
                                currentAssignment.Where(esrAssignment => assignment1 != null && esrAssignment.AssignmentID == assignment1.AssignmentID && esrAssignment.EffectiveStartDate == assignment1.EffectiveStartDate))
                            {
                ...
                if (primaryAssignment != null)
                {
                        ...
                        if (employee != null)
                        {
                            foreach (FieldInfo field in employee.ClassFields())
                            {
                                ...
                                if (valueSet)
                                {
                                    ...
                                    this.EsrApiHandler.Execute(DataAccessMethod.Update, "", new List<Employee> { employee });
                                    break;
                                }
                            }
                        }
                    ...
                    if (employee.linemanager == null && template != null)
                    {
        ...
        private int? GetEmployeeId(Guid fieldId, EsrAssignment assignment)
        {
            ...
            switch (fieldId.ToString())
            {
                case "d68db9d1-0a73-4a76-aae9-f0de5f19f9ff":
                    ...
                    if (assignment != null)
                    {
                        personId = assignment.SafeSupervisorPersonId;
                    }
                    break;
        ...
        /// The <see cref="Car"/>.
        /// </returns>
        private Car FindCarByRegistration(string registration, IEnumerable<Car> cars, List<CarAssignmentNumberAllocation> carAssignments, long vehicleAllocationId)
        {
            foreach (Car car in cars)
            {
                var assignments = this.FindCarAssignment(car.carid, carAssignments);
                if (assignments != null && assignments.Count > 0)
                {
                    if (assignments.Any(assignmentNumberAllocation => car.registration == registration && assignmentNumberAllocation.ESRVehicleAllocationId == vehicleAllocationId))
                    {
                        return car;
                    }
                }
